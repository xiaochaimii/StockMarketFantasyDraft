...
# Load pre-fetched stock data from JSON (generated by fetch_data.py / GitHub Action)
_STOCK_DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "stock_data.json")
_prefetched = {}
try:
    with open(_STOCK_DATA_PATH) as _f:
        _prefetched = json.load(_f)
...
def fetch_returns(tickers, start, end):
    ...
    price_data = _prefetched["prices"]
    # Build DataFrame from JSON
    frames = {}
    for t in tickers:
        if t in price_data:
            frames[t] = {pd.Timestamp(d): v for d, v in price_data[t].items()}
    if not frames:
        return None, None, None
...
def fetch_dividends(tickers, start, end):
    """Read dividends from pre-fetched JSON data."""
    divs = _prefetched.get("dividends", {})
    return {t: divs.get(t, 0.0) for t in tickers}


def compute_signals(tickers, start, end):
    """Read technical signals from pre-fetched JSON data."""
    sigs = _prefetched.get("signals", {})
    return {t: sigs[t] for t in tickers if t in sigs}


def fetch_news_batch(tickers_tuple):
    """Read news from pre-fetched JSON data."""
    news_data = _prefetched.get("news", {})
    results = []
    for t in tickers_tuple:
        items = news_data.get(t, [])
        if items:
            item = items[0]
            results.append({"ticker": t, "title": item.get("title", ""), "publisher": item.get("publisher", ""), "link": item.get("link", "")})
    return results


def fetch_earnings(tickers_tuple):
    """Read earnings from pre-fetched JSON data."""
    earn_data = _prefetched.get("earnings", {})
    ...
    def live_dashboard():
        ...
        if is_market_open():
            ...
            _status_bar = (
                ...
                'border-radius:10px;padding:0.25rem 0.7rem;box-shadow:0 1px 6px rgba(0,0,0,0.03);">'
            ...
            components.html(_status_bar, height=50, scrolling=False)
        else:
        ...
        _lb_html = (
            ...
            '  .controls-bar .search-wrap input{font-size:14px !important;padding:0.15rem 0.3rem 0.15rem 1.4rem !important;border-radius:6px !important;}'
            '  .controls-bar .search-wrap .s-icon{font-size:0.65rem !important;left:0.35rem !important;}'
            '  .controls-bar .search-wrap{margin-bottom:0.15rem !important;}'
            '  .controls-bar .vdiv{display:none;}'
            '  .controls-bar .date-group{width:100%;justify-content:space-between;gap:0.2rem !important;}'
            '  .controls-bar .date-input{flex:1;min-width:0;font-size:14px !important;padding:0.15rem 0.2rem !important;border-radius:6px !important;}'
